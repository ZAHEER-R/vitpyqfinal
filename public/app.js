...
function load() {
  ...
  const uid = localStorage.getItem('vitpyq_uid');
  if (uid) state.user = state.users.find(u => u.id === uid) || null;
  save();
}
...
function isVitEmail(email) {
  return email && (email.toLowerCase().endsWith('@vitstudent.ac.in') || email === ADMIN_EMAIL);
}
...
function googleLogin() {
  const email = prompt('Enter your VIT email (@vitstudent.ac.in):');
  if (!email) return;
  if (!isVitEmail(email)) { toast('Only @vitstudent.ac.in emails allowed', 'error'); return; }
  ...
  if (!user) {
    ...
    user = {
      ...
      branch: 'VIT', email: email.toLowerCase(), password: '',
      displayName: uname, avatar: '', badge: 'bronze',
      vcash: 0, uploads: 0, downloads: 0, visits: 1, isAdmin: false, items: [], friends: [], requests: []
    };
    state.users.push(user);
  } else user.visits = (user.visits || 0) + 1;
  state.user = user;
  save(); updateUI();
...
function handleRegister(e) {
  ...
  const user = {
    ...
    displayName: first + ' ' + last, avatar: '', badge: 'bronze',
    vcash: 0, uploads: 0, downloads: 0, visits: 1, isAdmin: false, items: [], friends: [], requests: []
  };
  state.users.push(user);
  state.user = user;
...
function updateUI() {
  ...
  const fs = document.getElementById('friendsSearchBtn');
  const nb = document.getElementById('notifBtn');
  if (fs) fs.style.display = (logged && !(state.user && state.user.isAdmin)) ? 'flex' : 'none';
  ...
  if (logged) {
    ...
    document.getElementById('sideVcash').textContent = (state.user.vcash || 0).toLocaleString() + ' VX';
  }
  document.getElementById('statStudents').textContent = state.users.filter(u => !u.isAdmin).length.toLocaleString();
  document.getElementById('statPapers').textContent = state.papers.length.toLocaleString();
}
...
function paperCardHtml(p, compact) {
  ...
  let thumb;
  if (p.fileData && isPdf) {
    const src = p.fileData.split('#')[0] + '#toolbar=0&navpanes=0&scrollbar=0&view=FitH&page=1&zoom=page-width';
    thumb = `<iframe src="${src}" title="preview" scrolling="no" style="overflow:hidden"></iframe><div class="thumb-cover"></div>`;
  } else if (p.fileData) {
  ...
  const liked = state.user && (
    (state.user.likedPapers || []).includes(p.id) ||
    (p.likedBy || []).includes(state.user.id)
  );
  return `<div class="paper-thumb-card">
    <div class="thumb-frame">${thumb}</div>
    <div class="thumb-body">
      <h3>${escapeHtml(p.subject)}</h3>
      <div class="thumb-meta">${escapeHtml(p.code)} · ${p.year} · ${escapeHtml(p.campus || 'Vellore')}</div>
...
function toggleLike(id) {
  ...
  if (!p) return;
  if (window.VITPYQ_BACKEND === 'firebase' && window.VitApi) {
    if (!state.user.likedPapers) state.user.likedPapers = [];
    const likedIndex = state.user.likedPapers.indexOf(id);
    if (likedIndex >= 0) state.user.likedPapers.splice(likedIndex, 1);
    else state.user.likedPapers.push(id);
    save();
    renderHomePapers();
    renderPapers();
    renderNewHighlight();
...
function renderNewHighlight() {
  ...
  track.innerHTML = fresh.map(p => paperCardHtml(p)).join('');
}
...
function updateStoreVcash() {
  const bal = state.user ? (state.user.vcash || 0) : 0;
  document.getElementById('storeVcash').innerHTML = `Your balance: <strong>${bal.toLocaleString()}</strong> VX`;
}
function buyBadge(badge, cost) {
  if (!state.user) { toast('Login required', 'error'); showSection('auth'); return; }
  if ((state.user.vcash || 0) < cost) { toast('Not enough VX', 'error'); return; }
...
const ITEM_LABELS = {
  ...
  wave_glow: 'Wave Glow', pixel_border: 'Pixel Border'
};
...
function renderLeaderboard() {
  ...
  list.innerHTML = sorted.slice(0, 50).map((u, i) => {
    ...
    const score = state.lbTab === 'uploads' ? (u.uploads||0) + ' uploads'
      : state.lbTab === 'vcash' ? (u.vcash||0).toLocaleString() + ' VX' : (u.downloads||0) + ' downloads';
...
function openUserModal(id) {
  ...
  document.getElementById('modalVcash').textContent = (u.vcash || 0).toLocaleString();
...
function renderProfile() {
  ...
  document.getElementById('profVisits').textContent = u.visits || 0;
  document.getElementById('profVcash').textContent = (u.vcash || 0).toLocaleString();
...
function handleForgot(e) {
  ...
  if (!isVitEmail(email)) { toast('Only @vitstudent.ac.in emails', 'error'); return; }
  ...
  if (!pendingOtp) {
    ...
    toast('OTP: ' + pendingOtp + ' (demo — shown once)', 'success');
    alert('Your OTP is: ' + pendingOtp + '\n(Demo mode — in production this is emailed)');
    return;
  }
...
function toggleFriendsList() {
  ...
  const ids = state.user.friends || [];
  if (!ids.length) {
    box.innerHTML = '<p class="muted" style="padding:0.4rem;">No friends yet. Use Search Friends in the header.</p>';
  } else {
...
function pushSystemNotif(userId, text) {
  const u = state.users.find(x => x.id === userId);
  if (!u) return;
  if (!u.systemNotifs) u.systemNotifs = [];
  u.systemNotifs.unshift({ id: 'sn' + Date.now(), text, ts: Date.now(), fromAdmin: true });
  if (state.user && state.user.id === userId) state.user = u;
  save();
}
...
function confirmTerminate() {
  if (!termTargetId || !state.user?.isAdmin) return;
  const msg = document.getElementById('termMessage').value.trim();
  if (!msg) { toast('Send a personal message before terminating', 'error'); return; }
  if (!confirm('Permanently delete this account? This cannot be undone.')) return;

  // notify first (stored under a ghost key won't work after delete — store pending banner by email)
  ...
  // termination notice for next login attempt by email
  let banned = JSON.parse(localStorage.getItem('vitpyq_terminated') || '{}');
  banned[email] = {
    message: 'Admin\'s message: You have been terminated. Your account was deleted successfully by the admin. Reason: ' + msg,
    ts: Date.now()
  };
  localStorage.setItem('vitpyq_terminated', JSON.stringify(banned));

  // remove user's papers optional? keep papers but strip uploader link
  state.papers.forEach(p => {
    if (p.uploaderId === termTargetId) p.uploaderId = 'deleted';
  });
  // remove from friends lists
  state.users.forEach(x => {
    if (x.friends) x.friends = x.friends.filter(id => id !== termTargetId);
    if (x.requests) x.requests = x.requests.filter(id => id !== termTargetId);
  });
  state.users = state.users.filter(x => x.id !== termTargetId);
  save();
  closeTermModal();
  renderAdminUsers();
  updateUI();
...
const _googleLogin = googleLogin;
googleLogin = function() {
  const email = prompt('Enter your VIT email (@vitstudent.ac.in):');
  if (!email) return;
  const banned = JSON.parse(localStorage.getItem('vitpyq_terminated') || '{}');
  if (banned[email.toLowerCase()]) {
    alert(banned[email.toLowerCase()].message);
    return;
  }
  // reuse original by temporarily setting prompt - call inner logic
  if (!isVitEmail(email)) { toast('Only @vitstudent.ac.in emails allowed', 'error'); return; }
  let user = state.users.find(u => u.email.toLowerCase() === email.toLowerCase());
  if (!user) {
    const uname = email.split('@')[0].replace(/[^a-z0-9]/gi,'').slice(0,12) || 'user';
...
/* Admin donate VX + congratulate */
function adminDonate(userId) {
  if (!state.user?.isAdmin) return;
  const amount = parseInt(prompt('Donate VX amount (permanent):', '1000'), 10);
  if (!amount || amount < 1) return;
  const u = state.users.find(x => x.id === userId);
  if (!u) return;
  u.vcash = (u.vcash || 0) + amount;
  pushSystemNotif(userId, 'Admin donated ' + amount + ' VX to your account.');
  save();
  renderAdminUsers();
  toast('Donated ' + amount + ' VX', 'success');
}
...
renderAdminUsers = function() {
  ...
  box.innerHTML = users.map(u => {
    return `<div class="admin-row">
      <div class="info">
        <strong>${escapeHtml(u.displayName)} (@${escapeHtml(u.username)})</strong>
        <span>${escapeHtml(u.email)} · ${escapeHtml(u.branch||'')} · ${(u.vcash||0).toLocaleString()} VX · ${u.uploads||0} uploads</span>
      </div>
      <button class="btn-outline sm" onclick="openUserModal('${u.id}')">View</button>
...
function buyVerified() {
  ...
  if ((state.user.vcash || 0) < 50000) { toast('Need 50,000 VX', 'error'); return; }
...
renderAdminUsers = function() {
  __rau();
  // inject Message buttons if not present
  const box = document.getElementById('adminUsers');
  ...
  box.innerHTML = users.map(u => {
    return `<div class="admin-row">
      ...
      <button class="btn-outline sm" onclick="openUserModal('${u.id}')">View</button>
      <button class="btn-outline sm" onclick="adminMessageUser('${u.id}')"><i class="fas fa-comment"></i> Message</button>
      <button class="btn-outline sm" onclick="adminCongratulate('${u.id}')"><i class="fas fa-award"></i> Congrats</button>
      <button class="btn-outline sm" onclick="adminDonate('${u.id}')"><i class="fas fa-coins"></i> Donate</button>
      <button class="btn-ghost" style="color:var(--red);border-color:var(--red);" onclick="openTermModal('${u.id}')">
        <i class="fas fa-user-slash"></i> Terminate
  ...
  }).join('') || '<p class="center muted">No users found.</p>';
};
...
/* Ensure vault button visible after any profile render */
document.addEventListener('DOMContentLoaded', () => {
  const tryShow = () => {
    const vb = document.getElementById('vaultOpenBtn');
    if (vb && state.user) vb.style.display = 'block';
  };
  setInterval(tryShow, 1500);
});
